    ...
    for (var i = 0; i < lengthLoop; i++) {
      ...
      if (nodeSelected.type === "openai" || nodeSelected.type === "gemini") {
        try {
          ...
          const aiSettings: IOpenAi = {
            ...
            maxTokens: Number(maxTokens) || (model.includes('gpt-4') || model.includes('gpt-4o') ? 16000 : 1000),
          ...
          // INICIAR CONTEXTO PERSISTENTE PARA ESTE TICKET
          const sessionId = `ticket_${ticket.id}`;
          await SessionContextManager.saveContext(sessionId, {
            conversationHistory: [],
            tags: [],
            summary: null,
          ...
          // Recarregar o ticket para garantir que o contato está incluído
          const ticketWithContact = await Ticket.findByPk(ticket.id, {
            include: ["contact"]
          });

          if (!ticketWithContact || !ticketWithContact.contact) {
            logger.error(`[AI NODE] Não foi possível carregar o contato do ticket ${ticket.id}`);
            continue;
          }
          ...
          // Chamar a IA para processar a mensagem atual imediatamente
          logger.info(`[${provider.toUpperCase()} NODE] Processando mensagem atual com a IA para o ticket ${ticket.id}`);
          await handleOpenAiFlow(
            aiSettings,
            msg || null, // Use o parâmetro msg da função, não a variável local indefinida
            wbot,
            ticketWithContact,
            ticketWithContact.contact,
            undefined, // mediaSent
            ticketTraking
          );

          // Parar a execução do fluxo, pois a IA assumiu
          break;
        ...
        for (var iLoc = 0; iLoc < nodeSelected.data.seq.length; iLoc++) {
          ...
          if (elementNowSelected.includes("message")) {
            ...
            if (dataWebhook === "") {
              ...
              singleBlockMsg = replaceMessages(bodyFor, details, dataWebhook, dataLocal, idTicket);
            }
      ...
      if (nodeSelected.type === "menu") {
        ...
        if (pressKey) {
          ...
          // console.log(`[MENU NODE] Mensagem processada: ${menuMessage.body}`);
