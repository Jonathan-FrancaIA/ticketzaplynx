...
const SendWhatsAppMessage = async ({
  ...
  try {
    ...
    // Create a minimal ticket-like object for formatBody since it expects a Ticket but we have a Contact
    const ticketLike = { contact } as any;
    const sentMessage = await wbot.sendMessage(
      jid,
      {
        text: formatBody(body, ticketLike),
        contextInfo: {
          forwardingScore: 0,
          isForwarded: false
        }
    ...
    if (err.message && err.message.includes("senderMessageKeys")) {
      try {
        ...
        const sentMessage = await wbot.sendMessage(jid, { text: formatBody(body, { contact } as any) });
