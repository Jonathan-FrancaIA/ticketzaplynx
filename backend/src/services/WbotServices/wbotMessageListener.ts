  ...
  try {
    ...
    if (matchingCampaign) {
      ...
      try {
        ...
        // Executar o fluxo
        console.log("!!!! ANTES DE CHAMAR ACTIONSWEBHOOKSERVICE:", JSON.stringify(msg, null, 2));
...
const handleOpenAi = async (
  ...
  let messagesOpenAi = [...conversationHistory]; // Copiar histórico para usar na IA
  ...
  if (msg.message?.conversation || msg.message?.extendedTextMessage?.text) {
    messagesOpenAi.unshift({ role: "system", content: promptSystem }); // Adiciona prompt do sistema no início
    ...
    await SessionContextManager.addMessage(sessionId, { role: "assistant", content: response }, ticket.id);
    ...
    // Analisar resposta da IA para detectar ações automáticas
    await analyzeAndExecuteActions(response, ticket, contact, {
      name: "OpenAI",
...
const analyzeAndExecuteActions = async (
  ...
  try {
    ...
    const queueTransferMatch = responseText.match(/ação:\s*transferir\s+para\s+a\s+fila\s+["']?([^"']+)["']?/i);
    if (queueTransferMatch) {
      ...
      // Remover a instrução de ação do texto original
      const updatedResponse = responseText.replace(queueTransferMatch[0], "").trim();
      if (updatedResponse !== responseText) {
        // Aqui poderíamos atualizar a mensagem original se necessário
      }
    }

    // Detectar adição automática de tags
    const tagMatch = responseText.match(/ação:\s*adicionar\s+tag\s+["']?([^"']+)["']?/i);
    if (tagMatch) {
    ...
    // Verificar keywords de ação na resposta (além das expressões regulares específicas)
    if (responseText.toLowerCase().includes("ação:")) {
      // Processar outros tipos de ações aqui conforme necessário
      
      // Exemplo: Detectar múltiplas ações em uma única resposta
      if (responseText.toLowerCase().includes("ação: encerrar ticket")) {
...
const handleMessage = async (
  ...
  try {
    ...
    if (msg.key.fromMe && (ticket.isBot || ticket.useIntegration)) {
      logger.info(
        `Mensagem manual recebida no ticket ${ticket.id}, desativando o bot.`
      );
      await ticket.update({
        isBot: false,
        useIntegration: false,
