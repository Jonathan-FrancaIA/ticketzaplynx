...
import ConversationSummarizer from "../../utils/conversationSummarizer";
import OpenAIMessage from "../../models/OpenAIMessage";
import SessionContextManager from "../../utils/sessionContextManager";
...
const processResponse = async (
  ...
  const transferMatch = response?.match(/\u200e?\s*ação:\s*transferir\s+para\s+o\s+setor\s+de\s+atendimento/i);
  if (transferMatch) {
    ...
    response = response.replace(transferMatch[0], "").trim();
...
// Função para analisar a resposta da IA e executar ações automáticas
const analyzeAndExecuteActions = async (
  responseText: string,
  ticket: Ticket,
  contact: Contact,
  aiSettings: IOpenAi
  ...
  try {
    let updatedResponse = responseText;
    
    // Detectar transferência automática para fila específica
    const queueTransferMatch = updatedResponse.match(/\u200e?\s*ação:\s*transferir\s+para\s+a\s+fila\s+['"]?([^'"]+)['"]?/i);
    if (queueTransferMatch) {
      const queueName = queueTransferMatch[1].trim();
      logger.info(`[AI SERVICE] IA solicitou transferência para a fila: ${queueName} - ticket ${ticket.id}`);
      ...
      if (queue) {
        // Atualizar o ticket com a nova fila e emitir evento WebSocket
        ...
        const sessionId = `ticket_${ticket.id}`;
        await SessionContextManager.setCurrentQueue(sessionId, queueName);
        
        logger.info(`[AI SERVICE] Ticket ${ticket.id} transferido automaticamente para a fila: ${queueName} (ID: ${queue.id})`);
      } else {
        logger.warn(`[AI SERVICE] Fila não encontrada: ${queueName} - ticket ${ticket.id}`);
      }
      
      // Remover a instrução de ação do texto
      updatedResponse = updatedResponse.replace(queueTransferMatch[0], "").trim();
    }

    // Detectar adição automática de tags
    const tagMatch = updatedResponse.match(/\u200e?\s*ação:\s*adicionar\s+tag\s+['"]?([^'"]+)['"]?\.?\s*/i);
    if (tagMatch) {
      const tagName = tagMatch[1].trim();
      logger.info(`[AI SERVICE] IA solicitou adição da tag: ${tagName} - ticket ${ticket.id}`);
      ...
      // Atualizar o ticket e emitir evento WebSocket via UpdateTicketService para garantir consistência
      await UpdateTicketService({ 
        ticketData: {}, // Apenas acionar a atualização para emitir o evento WebSocket
        ticketId: ticket.id, 
        companyId: ticket.companyId 
      });
      ...
      // Atualizar contexto da sessão
      const sessionId = `ticket_${ticket.id}`;
      await SessionContextManager.addTag(sessionId, tagName);
      
      logger.info(`[AI SERVICE] Tag ${tagName} adicionada ao ticket ${ticket.id}`);
      
      // Remover a instrução de ação do texto
      updatedResponse = updatedResponse.replace(tagMatch[0], "").trim();
    }

    // Detectar encerramento de ticket
    const closeTicketMatch = updatedResponse.match(/\u200e?\s*ação:\s*encerrar\s+ticket\.?\s*/i);
    if (closeTicketMatch) {
      logger.info(`[AI SERVICE] IA solicitou encerramento do ticket ${ticket.id}`);
      // Atualizar o ticket com status encerrado e emitir evento WebSocket
      await UpdateTicketService({ 
        ticketData: { 
          status: "closed",
          useIntegration: false,
      ...
      // Remover a instrução de ação do texto
      updatedResponse = updatedResponse.replace(closeTicketMatch[0], "").trim();
    }

    return updatedResponse;
  } catch (error) {
    logger.error("[AI SERVICE] Erro ao analisar e executar ações da IA:", error);
    return responseText; // Retorna o texto original em caso de erro
  }
};
...
if (minutesElapsed >= aiSettings.completionTimeout) {
    ...
    - Você é um agente de IA avançado, projetado para ser autônomo e inteligente.
    - Seu objetivo é resolver as necessidades do cliente de forma eficiente, usando as ferramentas e informações disponíveis.
    - Use o nome ${clientName} para um atendimento pessoal e gentil.
    - Responda de forma completa, sem cortes, e sempre termine suas frases.
    - Limite suas respostas a ${aiSettings.maxTokens} tokens.
    - **Nunca** anuncie as ações que você está realizando (como transferir, adicionar tags, etc.). As ações devem ser executadas de forma implícita. Apenas continue a conversa naturalmente após uma ação.

    Ações Automáticas Disponíveis:
    - Para transferir o cliente para uma fila específica, use o formato: "Ação: transferir para a fila 'NOME_DA_FILA'".
    - Para adicionar uma tag ao ticket, use o formato: "Ação: adicionar tag 'NOME_DA_TAG'".
    - Para encerrar o ticket, use o formato: "Ação: encerrar ticket".

    Instruções do Prompt Específico:
    ${aiSettings.prompt}

    Analise a conversa e o prompt específico com atenção. Execute as ações quando necessário e responda ao cliente de forma clara e objetiva.`;

    // Processar mensagem de texto
    if (bodyMessage) {
      // Pegar contexto persistente para este ticket
      const sessionId = `ticket_${ticket.id}`;
      let context = await SessionContextManager.getContext(sessionId);
      
      // Adicionar a nova mensagem ao contexto
      await SessionContextManager.addMessage(sessionId, { role: "user", content: bodyMessage }, ticket.id);
      
      // Verificar se é necessário resumir a conversa
      if (ConversationSummarizer.shouldSummarize(context.conversationHistory.length)) {
        const summary = await ConversationSummarizer.createSummary(context.conversationHistory, ticket.id);
        context.summary = summary.summary;
        await SessionContextManager.saveContext(sessionId, context);
      }
      
      // Obter histórico de conversa com contexto persistente
      const conversationHistory = await SessionContextManager.getConversationHistory(sessionId);
      const messagesAI = [...conversationHistory]; // Copiar histórico para usar na IA

      // Adicionar prompt do sistema para OpenAI (não para Gemini)
      if (!isGeminiModel) {
      ...
      try {
        ...
        // Analisar resposta da IA para detectar ações automáticas e obter o texto limpo
        const cleanedResponse = await analyzeAndExecuteActions(responseText, ticket, contact, aiSettings);
        
        // Adicionar resposta da IA limpa ao contexto (garantir que não contenha ações)
        await SessionContextManager.addMessage(sessionId, { role: "assistant", content: cleanedResponse }, ticket.id);
        
        logger.info(`[AI SERVICE] Resposta processada com sucesso para ticket ${ticket.id}`);

        // Processar a resposta limpa para o cliente (substitui a resposta original)
        await processResponse(cleanedResponse, wbot, msg, ticket, contact, aiSettings, ticketTraking);
